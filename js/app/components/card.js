...
export const card = (() => {
    ...
    const renderReadMore = (uuid, uuids) => {
        ...
        return `<a class="text-theme-auto" style="font-size: 0.8rem;" onclick="undangan.comment.showOrHide(this)" data-uuid="${uuid}" data-uuids="${util.escapeHtml(uuids.join(','))}" data-show="${hasId ? 'true' : 'false'}" role="button" class="me-auto ms-1 py-0">${hasId ? 'Ocultar respuestas' : `Ver respuestas (${uuids.length})`}</a>`;
    };
    ...
    const renderBody = async (c) => {
        ...
        ${moreMaxLength ? `<p class="d-block mb-2 mt-0 mx-0 p-0"><a class="text-theme-auto" role="button" style="font-size: 0.85rem;" data-show="false" onclick="undangan.comment.showMore(this, '${c.uuid}')">Ver más</a></p>` : ''}`;
    };
    ...
    const renderReply = (id) => {
        ...
        <div class="d-block mb-2" id="comment-form-${id}">
            <div class="position-relative">
                ...
                <textarea dir="auto" class="form-control shadow-sm rounded-4 mb-2" id="form-inner-${id}" minlength="1" maxlength="1000" placeholder="Escribe una respuesta..." rows="3" data-offline-disabled="false"></textarea>
            </div>
        </div>
    ...
    const renderEdit = (id, presence, is_parent, is_gif) => {
        ...
        <select class="form-select shadow-sm mb-2 rounded-4" id="form-inner-presence-${id}" data-offline-disabled="false">
            <option value="1" ${presence ? 'selected' : ''}>&#9989; Asistiré</option>
            <option value="2" ${presence ? '' : 'selected'}>&#10060; No podré asistir</option>
        </select>`}
        ${!is_gif ? `<textarea dir="auto" class="form-control shadow-sm rounded-4 mb-2" id="form-inner-${id}" minlength="1" maxlength="1000" placeholder="Escribe tu comentario..." rows="3" data-offline-disabled="false"></textarea>    
        ` : `${!gif.isActive() ? '' : `<div class="d-none mb-2" id="gif-form-${id}"></div>`}`}
        <div class="d-flex justify-content-end align-items-center mb-0">
            <button style="font-size: 0.8rem;" onclick="undangan.comment.cancel(this, '${id}')" class="btn btn-sm btn-outline-auto rounded-4 py-0 me-1" data-offline-disabled="false">Cancelar</button>
            <button style="font-size: 0.8rem;" onclick="undangan.comment.update(this)" data-uuid="${id}" class="btn btn-sm btn-outline-auto rounded-4 py-0" data-offline-disabled="false">Actualizar</button>
        </div>`;
